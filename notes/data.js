...
// 注意：始终执行解析（不做 HTML 检测短路）。内容由我们手写，可含 <em>/<code> 等文本。
function parseNoteContent(text) {
  if (!text) return '';
  let html = String(text);

  // 1) 代码块 ```...``` —— 先行处理，保护其中内容不被后续规则污染
  ...
  // 6) 列表：先有序再无序；相邻 <li> 合并成 <ul>/<ol>
  html = html.replace(/^(\d+)\.[ \t]+(.+)$/gm, '<li data-ol="1">$2</li>');
  html = html.replace(/^[*-][ \t]+(.+)$/gm, '<li>$1</li>');
  // 相邻 li（含 data-ol 的）包成 <ol>
  ...
  html = html.replace(/\*([^*\s][^*]*?)<\/em>(?=\s|\)|,|，|。|！|!|\?|？)/g, '<em>$1</em>');
  // 更稳健的斜体：*word* —— 非空格字符包围，且至少一个字符
  html = html.replace(/(^|[^*])\*([^*\s][^*]*?)\*(?=[^*]|$)/g, '$1<em>$2</em>');
  // ==highlight==
  html = html.replace(/==([^=\n]+)==/g, '<mark>$1</mark>');

  // 8) 段落：以双换行分块；仅对纯文本（不含任何块级标签）的段落包 <p>
  //    避免 `<p></ul><blockquote>...</blockquote></p>` 这种非法嵌套
  const BLOCK_RE = /<(?:h[23]|p|pre|blockquote|hr|ul|ol|li)\b/i;
  html = html.split(/\n{2,}/).map(function(block) {
    block = block.trim();
    if (!block) return '';
    // 已经以块级标签开头，或包含块级标签：保持原样
